...
from utils_O.utils import generate_bbox, py_nms, convert_to_square
from utils_O.utils import pad, calibrate_box, processed_image
# 创建参数解析器
...
if not args.image_paths:
    ...
    with open(file_path, "r") as file:
        for line in file:
            ...
            four = list(map(lambda x: int(float(x)), part[1:5]))   # 四个坐标
            four_xy.append(four)

            ten = list(map(lambda x: int(float(x)), part[5:]))
            ten_xy.append(ten)
...
def compute_iou(true_box, pred_box):
    ...
    计算真实框和预测框的 IoU 值
    """
    x1_true, x2_true, y1_true, y2_true = true_box
    y1_pred, x1_pred, x2_pred, y2_pred = pred_box[:4]
    y1_true, x2_true, x1_true, y2_true = y1_true, x2_true, x1_true, y2_true
...
def compute_keypoint_accuracy(true_keypoints, pred_keypoints, threshold=5.0):
    """
    计算关键点的准确率，基于欧氏距离误差
    """
...
def evaluate_batch(true_boxes, pred_boxes, true_keypoints, pred_keypoints, iou_threshold=0.5, kp_threshold=10.0):
    """
    ...
    with open("output_1.txt", "w") as file_1:
        # 确保将 id_list 中的所有元素转换为字符串
        file_1.write(" ".join(map(str, id_list)))  # 用空格连接每个元素
    # 计算 IoU 平均值
