...
parser.add_argument('--image_path', type=str, default='/...',  help='需要预测图像的路径')
...
def draw_face(image_path, boxes_c, landmarks):
    ...
    for i in range(landmarks.shape[0]):
        for j in range(len(landmarks[i]) // 2):
            cv2.circle(img, (int(landmarks[i][2 * j]), int(landmarks[i][2 * j + 1])), 2, (0, 0, 255))
