...
def crop_landmark_image(data_dir, data_list, size, argument=True, M="12"):
    """裁剪并保存带有人脸关键点的图片
    参数：
      data_dir：数据目录
      size:裁剪图片的大小
    ...
    # 数据输出路径
    output = os.path.join(data_dir, M)
    ...
    for (imgPath, box, landmarkGt) in tqdm(data_list):
        ...
        for i in range(len(F_imgs)):
            ...
            if np.sum(np.where(F_landmarks[i] >= 1, 1, 0)) > 0:
                continue
            # 保存裁剪带有关键点的图片
            cv2.imwrite(os.path.join(dstdir, '%d.png' % (image_id)), F_imgs[i])
            # 把图片路径和label，还有关键点保存到数据列表上
            landmarks = list(map(str, list(F_landmarks[i])))
            f.write(os.path.join(dstdir, '%d.png' % (image_id)) + ' -2 ' + ' '.join(landmarks) + '\n')
            image_id += 1
    f.close()
...
# 合并图像后删除原来的文件
def delete_old_img(old_image_folder, image_size, M="12"):
    shutil.rmtree(os.path.join(old_image_folder, M, 'positive'), ignore_errors=True)
    shutil.rmtree(os.path.join(old_image_folder, M, 'negative'), ignore_errors=True)
    shutil.rmtree(os.path.join(old_image_folder, M, 'part'), ignore_errors=True)
    shutil.rmtree(os.path.join(old_image_folder, M, 'landmark'), ignore_errors=True)
    
    # 删除原来的数据列表文件
    os.remove(os.path.join(old_image_folder, M, 'positive.txt'))
    os.remove(os.path.join(old_image_folder, M, 'negative.txt'))
    os.remove(os.path.join(old_image_folder, M, 'part.txt'))
    os.remove(os.path.join(old_image_folder, M, 'landmark.txt'))
...
def save_hard_example(data_path, save_size):
    ...
    for im_idx, dets, gts in zip(im_idx_list, det_boxes, gt_boxes_list):
        ...
        # 读取原图像
        img = cv2.imread(im_idx, cv2.IMREAD_UNCHANGED)
