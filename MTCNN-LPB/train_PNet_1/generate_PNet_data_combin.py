...
from utils_1.utils import IOU, combine_data_list, crop_landmark_image, delete_old_img
from utils_1.utils import get_landmark_from_lfw_neg
...
def crop_12_box_image(data_path):
    npr = np.random
    # face的id对应label的txt
    anno_file = os.path.join(data_path, 'wider_face_train_1.txt')
    # 图片地址
    im_dir = os.path.join(data_path, 'WIDER_train/images')

    # pos，part,neg裁剪图片放置位置
    pos_save_dir = os.path.join(data_path, '13/positive')
    part_save_dir = os.path.join(data_path, '13/part')
    neg_save_dir = os.path.join(data_path, '13/negative')
    # PNet数据地址
    save_dir = os.path.join(data_path, '13/')
    ...
    for annotation in tqdm(annotations):
        ...
        # imread:用于读取图像文件并将其加载为 NumPy 数组

        img = cv2.imread(os.path.join(im_dir, im_path + '.jpg'))
        gray_img = extract_combined_elbp_dlbp(img)
        idx += 1
        gray_img = gray_img[:, :, np.newaxis]  # shape: (height, width, 1)

        # 在通道维度 (axis=2) 上拼接彩色原图和灰度图
        ...
        while neg_num < 50:
            ...
            if np.max(Iou) < 0.3:
                ...
                f2.write(neg_save_dir + '/%s.PNG' % n_idx + ' 0\n')
        ...
        for box in boxes:
            ...
            for i in range(20):
                ...
                if iou >= 0.65:
                    ...
                    f1.write(pos_save_dir + '/%s.png' % p_idx + ' 1 %.2f %.2f %.2f %.2f\n' % (offset_x1,
                                                                                              offset_y1, offset_x2,
                                                                                              offset_y2))
                    cv2.imwrite(save_file, resized_im)
                    p_idx += 1
                # iou值大于于0.4小于0.65判定为part图像
                    ...
                    f3.write(part_save_dir + '/%s.png' % d_idx + ' -1 %.2f %.2f %.2f %.2f\n' % (offset_x1,
                                                                                                offset_y1, offset_x2,
                                                                                                offset_y2))
                    cv2.imwrite(save_file, resized_im)
                    d_idx += 1
...
if __name__ == '__main__':
    ...
    crop_landmark_image(data_path, data_list, 12, argument=True, M="13")
    # 合并数据列表
    print('开始合成数据列表')
    combine_data_list(os.path.join(data_path, '13'))
    # 合并图像数据
    print('开始合成图像文件')
    convert_data(os.path.join(data_path, '13'), os.path.join(data_path, '13', 'all_data'))
    # 删除旧数据
    print('开始删除旧的图像文件')
    delete_old_img(data_path, 12, M="13")
